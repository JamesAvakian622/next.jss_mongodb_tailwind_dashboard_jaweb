 ...
 *
 * Sits next to the user avatar/dropdown. Hidden on the smallest screens
 * to avoid crowding the mobile header — visible from `sm` and up.
 */
const SOCIAL_LINKS = [
  {
    label: "Facebook",
      ...
      {SOCIAL_LINKS.map((s) => (
        <li key={s.label}>
          <a
            ...
            aria-label={s.label}
            className="text-body transition hover:text-primary dark:text-bodydark dark:hover:text-white"
          >
            <svg
              className="h-5 w-5"
              fill="currentColor"
